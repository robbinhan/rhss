...
| ✅ 候选-C | 成本感知 placement + `rhss cost` | 详见 D26。每个 backend 可选 `cost_per_gb_month`;`CostAwarePlacement` 挑最便宜 + 够空间的 backend;无 priced 时 fallback MostFree。`rhss cost` 出每月账单估算。**未做**:出口费 / 请求费 / Glacier 取回 / 阶梯定价 |
