...
| D26 | **成本感知 placement + `rhss cost` 命令(候选-C)** | 每个 backend 可选 `cost_per_gb_month`(env-friendly 浮点 USD)。新 `CostAwarePlacement`:挑成本最低且 `free_bytes >= min_free_bytes`(默认 1 GiB)的 backend;有任意 priced backend 时,未声明成本的 backend 视作无穷贵;**全无 priced 时 fallback 到 MostFree**(避免空跑失败)。配置 `placement = "cost_aware"`。`rhss cost` 命令:per-backend 表格 used×rate=monthly + 总计估算 + 标识未声明成本的 backend。同时支持 `--json`。**未做(留 v2)**:出口费(egress)、API 请求费(per 1000 ops)、Glacier 取回费用、按文件大小阶梯定价 |
